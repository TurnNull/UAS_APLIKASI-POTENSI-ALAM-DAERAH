        ...
        <div class="flex flex-wrap">
            <div class="w-full self-center px-4 lg:w-1/2">
                <h1 class="text-base font-semibold text-primary md:text-xl">
                    Hallo semuanya~, Saya <span class="block font-bold text-dark text-4xl lg:text-5xl mt-1">{{ $profil->nama }}</span>
                </h1>
                <h2 class="font-medium text-secondary text-lg mb-5 leading-relaxed lg:text-2xl">
                    {{ $profil->kelas }} | <span class="type-myTitle text-dark"></span></h2>
                @if($profil->alamat)
                    <p class="font-medium text-secondary mb-10">
                        Saya tinggal di <span class="text-dark">{{ $profil->alamat }}</span>
                    </p>
                @endif
                <a href="" class="text-base font-semibold text-white bg-primary py-3 px-8 rounded-full hover:bg-secondary hover:shadow-lg hover:opacity-80 transition duration-300 ease-in-out">Klik saya</a>
            </div>
            <div class="w-full relative self-end px-4 lg:w-1/2">
                <div class="w-full relative mx-auto h-auto overflow-hidden rounded-xl mt-10 lg:mt-0 lg:right-0 drop-shadow-md lg:hover:rounded-bl-[24rem] lg:hover:drop-shadow-[0_35px_35px_rgba(0,0,0,0.30)] lg:transition-all lg:duration-700">
                    <img src="{{ asset('storage/'. $profil->foto_profil) }}" alt="profile_image" class="w-full h-auto relative mx-auto rounded-3xl scale-110 transition-all duration-500 hover:scale-100">
                </div>
            </div>
        </div>
    </div>
...
<section id="about" class="py-36 mb-36 bg-dark text-white">
    <div class="container">
        <div class="flex flex-wrap">
            <div class="w-full px-4 mb-10 lg:w-1/2 text-end">
                <h4 class="font-bold uppercase text-start text-primary text-lg mb-3">Tentang Saya</h4>
                <h2 class="font-bold text-white text-3xl mb-5 max-w-xl lg:text-4xl">Lorem ipsum dolor sit amet consectetur adipisicing elit.</h2>
                @if ($profil->deskripsi)
                    <p class="font-medium text-base text-primary max-w-xl lg:text-lg">{{ $profil->deskripsi }}</p>
                @else
                    <p class="font-medium text-base text-primary max-w-xl lg:text-lg">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae accusamus optio nisi non quos repudiandae asperiores, dolorum odio officia nostrum aspernatur iure facere eum ducimus iste reprehenderit mollitia incidunt omnis.</p>
                @endif
            </div>
            {{-- <div class="w-full px-4 lg:w-1/2 text-end">
                <h3 class="font-semibold text-2xl mb-4 lg:text-3xl lg:pt-10">Ikuti saya</h3>
...
@section('footerInfo')
    <div class="w-full mb-12 md:w-1/3 text-center">
        <h2 class="font-extrabold text-4xl mb-3">{{ $profil->nim }}</h2>
    </div>
@endsection